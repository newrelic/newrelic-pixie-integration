...
Once the New Relic plugin is successfully enabled and the PXL scripts are registered, the integration exist with error code 0.
 ...
 * addExcludes (optional boolean, `false` by default): add pod and namespace includes to the custom script

Example of a custom script, eg. `/scripts/custom1.yaml`:
```
name: "custom1"
...
The `addExcludes` adds a filtering section to the script right before the call to `px.export`. The filtering code requires your PXL script to:
   * Use `df` as the exported dataframe
   * The dataframe `df` to have a `pod` field
   * The dataframe `df` to have a `namespace` field
